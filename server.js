...
`)).then(() => pool.query(`
  ALTER TABLE leads_historial ADD COLUMN IF NOT EXISTS color_tag TEXT DEFAULT 'blanco'
`)).catch(console.error);
    ...
    for (const e of entries) {
      ...
      const r = await pool.query(
        `INSERT INTO leads_historial
          (numero,categoria,fecha_pub,dia_horario,contexto,fecha_contacto,metodo,sms_enviado,fecha_sms,seguimiento,nota,color_tag)
        ...
        [e.numero, e.categoria||null, e.fecha_pub||null, e.dia_horario||null,
         ...
         !!e.sms_enviado, e.fecha_sms||null, e.seguimiento||'pendiente', e.nota||null, e.colorTag||'blanco']
      );
      inserted += r.rowCount;
    }
...
app.post('/api/colortag', async (req, res) => {
  const { numero, fecha_contacto, colorTag } = req.body || {};
  if (!numero || !fecha_contacto || !colorTag) return res.status(400).json({ error: 'faltan datos' });
  try {
    await pool.query(
      'UPDATE leads_historial SET color_tag=$1 WHERE numero=$2 AND fecha_contacto=$3',
      [colorTag, numero, fecha_contacto]
    );
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/all', async (req, res) => {
  try {
    const r = await pool.query(
      'SELECT numero,categoria,fecha_pub,dia_horario,contexto,fecha_contacto,metodo,sms_enviado,fecha_sms,seguimiento,nota,color_tag AS "colorTag" FROM leads_historial ORDER BY fecha_contacto'
    );
    res.json(r.rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
